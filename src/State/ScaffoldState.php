 ...
 *
 * State format 2 is intentionally strict. An existing current state file always
 * records a valid environment alongside its package baselines. Normal lifecycle
 * reads reject older formats. The explicit migrate flow may inspect known v1 state
 * only to validate and back it up; legacy package baselines are never imported.
 *
 * Safety model:
...
final class ScaffoldState {
	...
	/**
	 * Validate whether the canonical state is eligible for explicit migration.
	 *
	 * Migration never consumes legacy package/file baselines. This method only proves
	 * that an existing state file is a known CitOmni installer v1 state before the
	 * migration flow backs it up and replaces it. A missing state is allowed so an
	 * interrupted migration can be resumed safely.
	 *
	 * @return int|null Legacy format version, or null when no state exists.
	 * @throws ConflictException When the application already has current v2 state.
	 * @throws InstallerException When the state is malformed, foreign, or unsupported.
	 */
	public function legacyVersionForMigration(): ?int {
		$path = $this->path();
		...
		try {
			...
			throw new InstallerException(\sprintf(
				'State file could not be read safely for migration: %s',
				$path
			), 0, $e);
		}

		if (!\is_array($data) || !\is_int($data['format_version'] ?? null)) {
			throw new InstallerException('State file is missing a valid integer format_version: ' . $path);
		}

		$version = $data['format_version'];
		if ($version === self::FORMAT_VERSION) {
			throw new ConflictException(\sprintf(
				'State file is already format_version %d; migrate is not applicable: %s',
				self::FORMAT_VERSION,
				$path
			));
		}
		if ($version !== 1) {
			throw new InstallerException(\sprintf(
				'State file format_version %d is not supported by migrate: %s',
				$version,
				$path
			));
		}
		if (($data['generated_by'] ?? null) !== self::GENERATOR) {
			throw new InstallerException('Legacy state was not generated by ' . self::GENERATOR . ': ' . $path);
		}
